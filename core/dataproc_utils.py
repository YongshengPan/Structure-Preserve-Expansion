...
def standard_normalization(IMG, remove_tail=True, divide='mean'):
    ...
    if remove_tail:
        nIMG = (nIMG - min) / 2.0
        nIMG = sitk.Cast(sitk.Minimum(nIMG-2.0, sitk.Sqrt(sitk.Maximum(nIMG-2.0, 0))), sitk.sitkFloat32) + 2.0
        # nIMG = (nIMG - min) / 2.0
        # nIMG = sitk.Cast(
        #     sitk.Minimum(nIMG - 1.5, sitk.Sigmoid(nIMG - 1.5, outputMaximum=0.5, outputMinimum=0)) + 1.5, sitk.sitkFloat32)
    else:
